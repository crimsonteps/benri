...
- 启动或再次打开应用时直接显示主窗口
- 默认使用上下方向键选择记录，选中后自动复制该记录正文，也可以在设置中关闭自动复制
- 左方向键进入分类列，上下切换分类；右方向键依次返回记录列并进入纯正文视图，顶部强调线会显示当前键盘活动列
- 搜索框有内容时，普通左右方向键用于移动文本光标；`⌘←` 和 `⌘→` 始终用于切换栏位
- 纯正文视图按 `Enter` 复制当前记录，按左方向键返回记录列
- `⌘N` 新建记录，`⌘E` 编辑记录，`Esc` 隐藏面板
- 主页面设置按钮可调整外观、全局唤起快捷键、自动复制和记录列表内容预览
