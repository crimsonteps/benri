...
- 启动或再次打开应用时直接显示主窗口
- 使用上下方向键选择记录，按 `Enter` 复制正文并立即隐藏窗口
- 记录列按左方向键进入内容编辑；`⌘←` 进入分类列，分类列使用上下方向键切换
- 按 `⌘Enter` 唤起当前记录的操作菜单，选择“删除记录”后按 `Enter` 删除
- 记录列表和内容区域支持右键删除记录
- 右方向键进入纯正文视图，顶部强调线会显示当前键盘活动列
- 搜索框有内容时，普通左右方向键用于移动文本光标；`⌘←` 和 `⌘→` 始终用于切换栏位
- `⌘N` 新建记录，`Esc` 隐藏面板
- 主页面设置按钮可调整外观、全局唤起快捷键和记录列表内容预览
- 记录列表默认隐藏正文预览，避免账号密码直接显示，可在设置中开启
